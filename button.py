    ...
    with column1:
        ...
        if 'auth_status' in st.session_state:
            try:
                ...
                repmax_weight = max(maxdf2['weight'])
                maxdf2 = maxdf2.reset_index()
                repmax_lift = maxdf2['lift'].iloc[0]
                repmax_min = min(maxdf2['weight'])
    ...
    if 'auth_status' in st.session_state:
        ...
        for lft in maxdf1['lift'].unique():
            ...
            for i in range(len(maxtable)):
                ...
                maxtable['95%'].iloc[ind] = round(float(maxlft) * 0.95, 2)
                maxtable['90%'].iloc[ind] = round(float(maxlft) * 0.90, 2)
                maxtable['85%'].iloc[ind] = round(float(maxlft) * 0.85,2)
                maxtable['80%'].iloc[ind] = round(float(maxlft) * 0.80,2)
                maxtable['75%'].iloc[ind] = round(float(maxlft) * 0.75,2)
                maxtable['70%'].iloc[ind] = round(float(maxlft) * 0.70,2)
                maxtable['65%'].iloc[ind] = round(float(maxlft) * 0.65,2)
                maxtable['60%'].iloc[ind] = round(float(maxlft) * 0.60,2)
                maxtable['55%'].iloc[ind] = round(float(maxlft) * 0.55,2)
                maxtable['50%'].iloc[ind] = round(float(maxlft) * 0.50,2)
                maxtable['45%'].iloc[ind] = round(float(maxlft) * 0.45,2)
                maxtable['40%'].iloc[ind] = round(float(maxlft) * 0.40,2)
                maxtable['35%'].iloc[ind] = round(float(maxlft) * 0.35,2)
                maxtable['30%'].iloc[ind] = round(float(maxlft) * 0.30,2)
                maxtable['25%'].iloc[ind] = round(float(maxlft) * 0.25,2)
                maxtable['20%'].iloc[ind] = round(float(maxlft) * 0.20,2)
                maxtable['15%'].iloc[ind] = round(float(maxlft) * 0.15,2)
                maxtable['10%'].iloc[ind] = round(float(maxlft) * 0.10,2)
                maxtable['5%'].iloc[ind]= round(float(maxlft) * 0.05,2)
            ind = ind + 1
        st.dataframe(maxtable.round(1).style.background_gradient())
        colored_header("Records")
        st.dataframe(maxdf1[['lift', 'weight', 'date']])
    else:
        st.markdown("![Alt Text](https://i0.wp.com/www.signsrxusa.com/wp-content/uploads/2020/07/Percentage-Charts-55-550.png?resize=1000%2C800&ssl=1)")
